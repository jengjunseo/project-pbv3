...
-- Do not delete directly from storage.objects: current Supabase protects Storage metadata tables.
-- This test runs against a disposable database and inserts transaction-local fixture rows that ROLLBACK removes.

-- Server owns timestamps and UTF-8 byte accounting.
select public.pb_v3_commit_slot(17::smallint, '가a', 'keep', null, repeat('a', 64));
...
select public.pb_v3_commit_slot(17::smallint, '가나다', 'keep', null, repeat('a', 64));
select pg_temp.assert_true(
...
declare
  ...
  committed jsonb;
begin
  prepared := public.pb_v3_prepare_upload(17::smallint, repeat('b',64), 'file.pdf', 'application/pdf', 1, object_path, clock_timestamp()+interval '10 minutes');
  ...
  committed := public.pb_v3_commit_slot(17::smallint, 'with file', 'replace', cap, repeat('b',64));
...
begin
  prepared := public.pb_v3_prepare_upload(17::smallint, repeat('c',64), 'new.pdf', 'application/pdf', 10, new_path, clock_timestamp()+interval '10 minutes');
  cap := (prepared->>'capability_id')::uuid;
  ...
  perform public.pb_v3_commit_slot(17::smallint,'replacement','replace',cap,repeat('c',64));
...
begin
  prepared := public.pb_v3_prepare_upload(18::smallint,repeat('d',64),'cancel.txt','text/plain',1,path,clock_timestamp()+interval '10 minutes');
  cap := (prepared->>'capability_id')::uuid;
  perform public.pb_v3_cancel_upload(cap,repeat('d',64));
  begin
    perform public.pb_v3_commit_slot(18::smallint,'x','replace',cap,repeat('d',64));
...
begin
  begin
    perform public.pb_v3_prepare_upload(19::smallint,fp,'a.txt','text/plain',1,'slots/20/423e4567-e89b-42d3-a456-426614174000/a.txt',clock_timestamp()+interval '10 minutes');
  exception when others then failed := true;
  end;
  perform pg_temp.assert_true(failed,'cross-slot path must fail');

  perform public.pb_v3_prepare_upload(19::smallint,fp,'a.txt','text/plain',1,'slots/19/523e4567-e89b-42d3-a456-426614174000/a.txt',clock_timestamp()+interval '10 minutes');
  perform public.pb_v3_prepare_upload(19::smallint,fp,'b.txt','text/plain',1,'slots/19/623e4567-e89b-42d3-a456-426614174000/b.txt',clock_timestamp()+interval '10 minutes');
  failed := false;
  begin
    perform public.pb_v3_prepare_upload(19::smallint,fp,'c.txt','text/plain',1,'slots/19/723e4567-e89b-42d3-a456-426614174000/c.txt',clock_timestamp()+interval '10 minutes');
  exception when others then failed := true;
  end;
...
select public.pb_v3_prepare_upload(0::smallint,repeat('f',64),'new.txt','text/plain',1,'slots/00/823e4567-e89b-42d3-a456-426614174000/new.txt',clock_timestamp()+interval '10 minutes');
